...
@section('contenido')
    <div class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
        <form name="as" method="POST" action="{{route('articulos.update',$articulo)}}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="mb-4">
                ...
                <input value="{{@old('titulo',$articulo->titulo)}}"
            ...
            <div class="mb-4">
                ...
                <textarea
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                    id="contenido" name="contenido" placeholder="Contenido...">{{@old('contenido',$articulo->contenido)}}</textarea>
                @error('contenido')
                    <p class="text-red-700 italic text-xs mt-2">***{{ $message }}</p>
                @enderror
            </div>
            <div class="mb-4">
                ...
                <div class="flex">
                    <div class="flex items-center mr-4">
                        ...
                        @checked (@old('estado', $articulo->estado) == "Publicado")
                    ...
                    <div class="flex items-center mr-4">
                        ...
                        @checked (@old('estado', $articulo->estado) == "Borrador")
                            class="w-4 h-4 text-blue-600 bg-gray-100 border-gray-300 focus:ring-blue-500 dark:focus:ring-blue-600 dark:ring-offset-gray-800 focus:ring-2 dark:bg-gray-700 dark:border-gray-600">
                        <label for="borrador"
            ...
            <div class="mb-4">
                ...
                <select id="user_id" name="user_id"
                    ...
                    @foreach ($autores as $item)
                        <option value="{{ $item->id }}" @selected(@old('user_id', $articulo->user_id) == $item->id)>{{ $item->name }}</option>
                    @endforeach
                </select>
